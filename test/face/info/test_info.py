...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            None,
            id="face.info: Clicking the info button"
        )
    ]
)
def test_info(runner, qtbot, mocker, _) -> None:
    ...
    assert runner.infoobjc.comp.text() == f"This version is compatible with Genshin Impact {__gicompat_vers__} Phase {__gicompat_part__}"
