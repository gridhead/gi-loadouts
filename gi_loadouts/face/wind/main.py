...
class MainWindow(Rule):
    ...
    def initialize_events(self) -> None:
        ...
        for item in [
            ...
            item[1].currentTextChanged.connect(
                lambda _, a_type=item[1], a_id=item[5]: self.change_artifact_team_by_changing_type(
                    a_type, a_id
                )
            )
            item[1].currentTextChanged.connect(
        ...
        for part in ["fwol", "pmod", "sdoe", "gboe", "ccol"]:
            for alfa in ["a", "b", "c", "d"]:
                drop, text = (
                    getattr(self, f"arti_{part}_name_{alfa}"),
                    getattr(self, f"arti_{part}_data_{alfa}"),
                )
            ...
            getattr(self, f"arti_{part}_scan").clicked.connect(
                lambda _, a_part=part: self.show_scan_dialog(a_part)
            )
            getattr(self, f"arti_{part}_load").clicked.connect(
                lambda _, a_part=part: self.arti_load(a_part)
            )
            getattr(self, f"arti_{part}_save").clicked.connect(
                lambda _, a_part=part: self.arti_save(a_part)
            )
            getattr(self, f"arti_{part}_wipe").clicked.connect(
                lambda _, a_part=part: self.wipe_artifact(a_part)
            )
