...
class GBOE(Artifact):
    ...
    def stat_data(self) -> float:
        ...
        if self.stat_name == MainStatType_GBOE.attack_perc:
            ...
            calc = calc = elemental_mastery[self.rare]["init"] + sum(
            ...
            calc = damage_bonus_elemental_perc[self.rare]["init"] + sum(
                damage_bonus_elemental_perc[self.rare]["diff"][0 : self.levl + 1]
            )  # noqa : E501
        elif self.stat_name == MainStatType_GBOE.damage_bonus_physical_perc:
            calc = damage_bonus_physical_perc[self.rare]["init"] + sum(
                damage_bonus_physical_perc[self.rare]["diff"][0 : self.levl + 1]
            )  # noqa : E501
        return calc
